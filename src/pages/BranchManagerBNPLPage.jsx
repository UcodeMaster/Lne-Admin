...
import {
  ...
  Eye, Calendar, ShieldCheck, AlertCircle,
  FileText, Loader2, RefreshCw, AlertTriangle, BarChart3, ClipboardList, X, Download,
} from 'lucide-react';
...
const statusCfg = {
  ...
  draft: { bg: C.gray100, color: C.gray500, border: C.gray200, Icon: FileText, label: 'Draft' },
};
...
const Field = ({ label, value }) => (
  <div>
    ...
    <div style={{ fontSize: 13.5, color: value ? C.gray700 : C.gray400, fontWeight: value ? 500 : 400, wordBreak: 'break-word' }}>{value || 'N/A'}</div>
  </div>
);
...
const BranchManagerBNPLPage = () => {
  ...
  const fetchApplications = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await client('/branch-manager/bnpl');
  ...
  const handleStatus = async (id, status, reason = null) => {
    try {
      ...
      if (selectedApp?.id === id) {
        setSelectedApp(prev => (prev ? { ...prev, status, rejection_reason: reason, reviewed_at: new Date().toISOString() } : prev));
      }
    } catch (err) {
  ...
  const counts = { all: applications.length, pending: 0, approved: 0, rejected: 0, draft: 0 };
  applications.forEach(a => { if (counts[a.status] !== undefined) counts[a.status]++; });

  const totalDeposits = applications.filter(a => a.status === 'approved').reduce((sum, a) => sum + parseFloat(a.deposit_amount ?? 0), 0);
    ...
    <DashboardLayout>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, gap: 12, flexWrap: 'wrap' }}>
        <div>
          ...
          <p style={{ color: C.gray500, marginTop: 4, fontSize: 13 }}>Review, approve, or reject applications within your branch location.</p>
        </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: '0' }}>
              <thead>
                <tr style={{ background: C.gray50 }}>
                  {['ID', 'Customer', 'Source', 'Package', 'Plan', 'Initial Payment', 'Status', 'KYC', 'Applied', 'Actions'].map((h, i) => (
                    <th key={h} style={{ padding: '14px 16px', fontSize: 12, color: C.gray500, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5, textAlign: i === 9 ? 'center' : 'left', whiteSpace: 'nowrap', borderBottom: `1px solid ${C.gray200}` }}>
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {filtered.map(app => {
                  ...
                  return (
                    <tr key={app.id} className="bnpl-row" style={{ borderBottom: `1px solid ${C.gray100}`, background: C.white, transition: 'background 0.15s' }}>
                      ...
                      <td style={{ padding: '13px 16px', minWidth: 180 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          ...
                          <div style={{ minWidth: 0 }}>
                            <div style={{ fontWeight: 700, color: C.dark, fontSize: 13, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 140 }}>
                              {app.user?.name || app.survey_data?.customer_name || '—'}
                            </div>
                            <div style={{ fontSize: 11, color: C.gray400, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 140 }}>
                              {app.user?.email || app.survey_data?.customer_email || '—'}
                            </div>
                          </div>
                        </div>
                      </td>

                      <td style={{ padding: '13px 16px', fontSize: 12, color: C.gray700, textTransform: 'capitalize', whiteSpace: 'nowrap' }}>
                        {(app.registration_source || '—').charAt(0).toUpperCase() + (app.registration_source || '—').slice(1)}
                      </td>
                      ...
                      <td style={{ padding: '13px 16px', textAlign: 'center', whiteSpace: 'nowrap' }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, flexWrap: 'wrap' }}>
                          <button
                            ...
                            style={{ padding: '6px 12px', borderRadius: 8, border: `1px solid ${C.gray200}`, background: C.white, color: C.gray600, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 12, fontWeight: 600 }}
                          >
                            <Eye size={14} /> View
                          </button>
                          {app.status === 'pending' && (
                            <button
                              ...
                              disabled={processing}
                              title="Approve"
                              style={{ padding: '6px 10px', borderRadius: 8, border: `1px solid ${C.primaryLight}`, background: C.primaryLight, color: C.primary, cursor: 'pointer', display: 'inline-flex', alignItems: 'center' }}
                            >
                          ...
                          {app.status === 'pending' && (
                            <button
                              ...
                              title="Reject"
                              style={{ padding: '6px 10px', borderRadius: 8, border: `1px solid ${C.errorLight}`, background: C.errorLight, color: C.error, cursor: 'pointer', display: 'inline-flex', alignItems: 'center' }}
                            >
                              <XCircle size={14} />
                            </button>
                          )}
      ...
      {panelOpen && selectedApp && (
        <>
          ...
          <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 'min(700px, 95vw)', background: C.white, zIndex: 901, overflowY: 'auto', boxShadow: '-8px 0 40px rgba(0,0,0,0.12)', animation: 'slideIn 0.25s ease', display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                {[
                  ...
                  { Icon: Package, label: 'Package', children: <>
                    ...
                    <div style={{ fontSize: 12, color: C.gray400 }}>{selectedApp.product?.capacity}{selectedApp.product?.voltage ? ` · ${selectedApp.product.voltage}` : ''}</div>
                  </> },
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
                  {Object.entries(REQUIRED_DOCS).map(([key, label]) => {
                    const uploaded = selectedApp?.survey_data?.uploaded_documents?.[key] || selectedApp?.survey_data?.documents?.[key];
                    return (
                      <div key={key} style={{ padding: 12, borderRadius: 10, border: `1.5px solid ${uploaded ? '#A5D6A7' : '#FFCDD2'}`, background: uploaded ? C.primaryLight : C.errorLight, display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'center', textAlign: 'center' }}>
                      ...
                      {docs.map((doc, i) => (
                        <div key={i} className="doc-card" style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 16px', borderRadius: 12, border: `1px solid ${C.gray200}`, background: C.white, transition: 'all 0.15s' }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <div style={{ fontSize: 11, color: C.gray400, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{doc.fileName || doc.url}</div>
                          </div>
                          <div style={{ display: 'flex', gap: 8, flexShrink: 0, flexWrap: 'wrap' }}>
                            <a href={doc.url} target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 12px', borderRadius: 8, border: `1px solid ${C.gray200}`, color: C.gray700, textDecoration: 'none', fontSize: 12, fontWeight: 600, background: C.white }}>
                              <Eye size={13} /> Preview
                            </a>
                            <a href={doc.url} download style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 12px', borderRadius: 8, border: `1px solid ${C.primary}`, color: C.primary, textDecoration: 'none', fontSize: 12, fontWeight: 600, background: C.primaryLight }}>
                  ...
                  {selectedApp.survey_data.personal && (
                    <div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 20px' }}>
                        ...
                        <div style={{ gridColumn: 'span 2' }}><Field label="Address Line 2" value={selectedApp.survey_data.personal?.address2} /></div>
                  ...
                  {selectedApp.survey_data.nok && (
                    <div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 20px' }}>
                        ...
                        <Field label="Middle Name" value={selectedApp.survey_data.nok?.middleName} />
                        ...
                        <Field label="Alt Phone" value={selectedApp.survey_data.nok?.phoneNumber2} />
                        ...
                        <div style={{ gridColumn: 'span 2' }}><Field label="Residential Address" value={selectedApp.survey_data.nok?.residentialAddress} /></div>
                        <div style={{ gridColumn: 'span 2' }}><Field label="Address Line 2" value={selectedApp.survey_data.nok?.address2} /></div>
                        <div style={{ gridColumn: 'span 2' }}><Field label="Office Address" value={selectedApp.survey_data.nok?.officeAddress} /></div>
                      </div>
                    </div>
                  )}
                  ...
                  {selectedApp.survey_data.banking && (
                    <div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 20px' }}>
                        ...
                        <Field label="Driving Licence No." value={selectedApp.survey_data.banking?.drivingLicenseNo} />
                        <Field label="Driving Licence Exp." value={selectedApp.survey_data.banking?.drivingLicenseExpDate} />
                        <Field label="Voters Card VIN" value={selectedApp.survey_data.banking?.votersCardVin} />
                      </div>
                    </div>
                  )}
                  ...
                  {selectedApp.survey_data.additional && (
                    <div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                        ...
                        <Field label="Referral Source & Contact" value={selectedApp.survey_data.additional?.referralSourceAndContact} />
                      </div>
                    </div>
                  )}
      ...
      {rejectModal && selectedApp && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: C.white, borderRadius: 20, width: '100%', maxWidth: 480, boxShadow: '0 20px 60px rgba(0,0,0,0.2)', overflow: 'hidden' }}>
            <div style={{ padding: '20px 24px', borderBottom: `1px solid ${C.gray200}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
              <div>
              ...
              <button onClick={closeRejectModal} style={{ width: 32, height: 32, borderRadius: '50%', border: `1px solid ${C.gray200}`, background: C.gray50, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.gray500 }}>
                <X size={16} />
              </button>
            </div>
            ...
            <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button
                  onClick={() => handleStatus(selectedApp.id, 'rejected', rejReason)}
...
export default BranchManagerBNPLPage;
